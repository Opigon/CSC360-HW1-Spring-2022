...
const Product = class Product {
    ...
    constructor(toParse){
        ...
        this.name = parsed[0];
        this.price = Number(parsed[1]);
        this.availability = parsed[2];
    }
    ...
    static inStock = (products) => {
         ...
         let prodInStock = products.filter(product => product.availability == "In Stock");
         return prodInStock;
    }
    ...
    static halfOff = (products) => {
        let retProd = products.slice();
        /*for (let i = 0; i < products.length; i++) {
            retProd[i] = products[i];
          }*/
        retProd.forEach((product) => {
            product.price *= 0.5;
        });
        return retProd;
    }
    ...
    static printProducts = (products) => {
        for (let product in products) {
            if (product.availability == "In Stock") {
                console.log("Product:  %s, Cost:  $%s, Available:  Yes", product.name,  product.price);
            }
            else {
                console.log("Product:  %s, Cost:  $%s, Available:  No", product.name,  product.price);
            }
        }
    }
